...
let rec eval_exp = function
  ...
  | S.Match (e, e1, x, xs, e2) ->
    begin match eval_exp e with
      | S.Nil-> eval_exp e1
      | S.Cons (x', xs') -> eval_exp (S.subst [(x, x'); (xs, xs')] e2)
...
let rec step = function
  | S.Var _ | S.Int _ | S.Bool _ | S.Lambda _ | S.RecLambda _ | S.Nil -> failwith "Expected a non-terminal expression 1"
  ...
  | S.Pair (e1, e2) when is_value e2 -> failwith "Expected a non-terminal expression 2"
  | S.Pair (e1, e2) when is_value e1 -> S.Pair (e1, step e2)
  | S.Pair (e1, e2) -> S.Pair (step e1, e2)
  | S.Fst e when is_value e ->
    begin
    match e with
    | S.Pair (v1, v2) -> v1
    | _ -> failwith "Pair expected"
  ...
  | S.Cons (e, es) when is_value es -> failwith "Expected a non-terminal expression 5"
  | S.Cons (e, es) when is_value e -> S.Cons (e, step es)
  | S.Cons (e, es) -> S.Cons (step e, es)
  | S.Match (e, e1, x, xs, e2) when is_value e ->
    begin
      match e with
      | S.Nil -> e1
      | S.Cons (x', xs') -> S.subst [(x, x'); (xs, xs')] e2
